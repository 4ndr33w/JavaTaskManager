package project.task.manager.user_service.security.component;
...
public class JwtTokenProvider {
		...
		public String createAccessToken(AppUserDetails userDetails) {
				...
				return JWT.create()
								...
								.withClaim("roles", userDetails.getUser().getRoles().stream()
												.map(Enum::name)
												.toList())
		...
		public DecodedJWT getDecodedAccessToken(String token) {
				try {
						...
						log.warn("{}: {}", Constant.ACCESS_TOKEN_EXPIRED_MESSAGE, e.getMessage());
						throw new TokenExpirationException(Constant.ACCESS_TOKEN_EXPIRED_MESSAGE, e);
				} catch (JWTVerificationException e) {
						log.warn("{}: {}", Constant.INVALID_ACCESS_TOKEN_MESSAGE, e.getMessage());
						throw new TokenValidationException(Constant.INVALID_ACCESS_TOKEN_MESSAGE, e);
				}
		}
		...
		public DecodedJWT getDecodedRefreshToken(String token) {
				try {
						...
						log.error("{}: {}", Constant.INVALID_REFRESH_TOKEN_MESSAGE, e.getMessage());
						throw new TokenValidationException(Constant.INVALID_REFRESH_TOKEN_MESSAGE, e);
				}
		}
		...
		public String getUserNameFromToken(String token) {
				try {
						...
						log.error("{}: {}", Constant.FAILED_TO_GET_USERNAME_FROM_TOKEN_MESSAGE, e.getMessage());
						throw new TokenValidationException(Constant.FAILED_TO_GET_USERNAME_FROM_TOKEN_MESSAGE, e);
				}
		}
		...
		public UUID getUserIdFromRefreshToken(String token) {
				try {
						...
						log.error("{}: {}", Constant.FAILED_TO_GET_USER_ID_FROM_TOKEN_MESSAGE, e.getMessage());
						throw new TokenValidationException(Constant.FAILED_TO_GET_USER_ID_FROM_TOKEN_MESSAGE, e);
				}
		}
		...
		public boolean validateRefreshToken(String token) {
				try {
						...
						log.warn("{}: {}", Constant.INVALID_REFRESH_TOKEN_MESSAGE, e.getMessage());
						return false;
				}
		}
